...
export function WhatsAppWidget() {
  // Hemen varsayılan config ile başla - böylece widget hemen görünür
  ...
  // Config fetch - arka planda güncelle
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        ...
        // Sadece geçerli veri geldiyse güncelle
        if (data && typeof data === 'object') {
          setConfig(prev => ({ ...prev, ...data }))
        }
      } catch (err) {
        // Zaten varsayılan config kullanılıyor, hata durumunda bir şey yapma
  ...
  useEffect(() => {
    ...
    }, 100) // Sadece 100ms gecikme

    return () => clearTimeout(timer)
  }, [config, isHydrated])

  // Hydration tamamlanmadan render etme (SSR/CSR mismatch önlemek için)
  if (!isHydrated) return null
  
  // Devre dışı veya görünmemesi gerekiyorsa
  if (config.enabled === false || !isVisible) return null
  ...
  return (
    <div 
      className={`fixed ${positionClasses[pos]} z-[9999] flex items-center`}
