...
export default async function SiteLayout({
  ...
  const waSize = waConfig?.button_size || 60
  const waShowTooltip = waConfig?.show_tooltip ?? true
  const waTooltip = waConfig?.tooltip_text || 'Bize WhatsApp\'tan yazın!'
  const waPulse = waConfig?.pulse_animation ?? true
  ...
  return (
    <html lang="tr" className={`${playfair.variable} ${inter.variable} scroll-smooth`}>
      <body className="font-sans antialiased bg-off-white text-gray-800">
        ...
        {waEnabled && (
          <a
            href={`https://wa.me/${waPhone.replace(/\D/g, '')}?text=${encodeURIComponent(waMessage)}`}
            target="_blank"
            ...
            style={{
              ...
              cursor: 'pointer',
              textDecoration: 'none',
              ...(positionStyles[waPosition] ? 
                Object.fromEntries(positionStyles[waPosition].split(';').filter(Boolean).map(s => {
                  const [k, v] = s.split(':');
                  return [k, v];
                })) 
          ...
          >
            <svg 
              viewBox="0 0 24 24" 
              style={{ width: `${waSize * 0.53}px`, height: `${waSize * 0.53}px`, fill: 'white' }}
            >
        ...
        <script
          dangerouslySetInnerHTML={{
            __html: `
              document.addEventListener('mouseover', function waHover() {
                var btn = document.getElementById('wa-widget-btn');
                if (btn) {
                  btn.style.transform = 'scale(1.1)';
                }
              });
              document.addEventListener('mouseout', function waOut() {
                var btn = document.getElementById('wa-widget-btn');
                if (btn) {
